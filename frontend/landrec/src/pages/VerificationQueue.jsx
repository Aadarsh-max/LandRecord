...
export default function VerificationQueue() {
  ...
  const displayFields = selectedRecord
    ? Object.keys(selectedRecord).filter((key) =>
        ["landowner_name", "survey_number", "khasra_number", "khata_number", "plot_area", "village", "tehsil", "district", "land_classification", "ownership_type", "mutation_status", "registration_number"].includes(key)
      )
    : [];
      ...
      <div className="flex flex-1">
        <div className="w-72 border-r border-ink-muted/10 px-5 py-8">
          ...
          <div className="space-y-2">
            ...
            {!loading && records.length === 0 && (
              <p className="text-sm text-ink-muted">No records yet. Upload a document to get started.</p>
            )}
          </div>
        </div>
        ...
        <div className="flex-1 px-10 py-8">
          {selectedRecord ? (
            ...
            <p className="text-ink-secondary">Select a record to review, or upload a new document.</p>
          )}
        </div>
      </div>
    </div>
