...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    api
      ...
      .then((response) => setRecent(response.data.recent));
  }, []);
      ...
      <div className="flex-1 px-5 py-8 sm:px-10 sm:py-10">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-ink-secondary">
              Live status across all districts
            </p>
          </div>
          ...
          <div className="flex flex-wrap gap-2">
            {quickActions.map(({ to, label, icon: Icon, variant }) => {
              const styleMap = {
                primary:
                  "bg-gradient-to-br from-blue-500 to-blue-600 text-white shadow-clay hover:shadow-claySm",
                secondary:
                  "bg-gradient-to-br from-amia-500 to-green-500 text-white shadow-clay hover:shadow-claySm",
                ghost:
                  "bg-base-surfaceLight text-ink-primary shadow-clay hover:shadow-clayInset",
              };
              return (
                <Link
                  key={to}
        ...
        <div className="mt-8 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4">
          {statCards.map(({ label, value, icon: Icon, tone }) => (
            <div
              key={label}
              className="rounded-clay bg-base-surfaceLight p-6 shadow-clay transition-transform duration-300 hover:-translate-y-1"
            >
              <div
                className={`inline-flex h-10 w-10 items-center justify-center rounded-claySm text-white ${toneStyles[tone]}`}
              >
        ...
        <div className="mt-6 grid grid-cols-1 gap-5 lg:grid-cols-3">
          <div className="rounded-clay bg-base-surfaceLight p-6 shadow-clay lg:col-span-2">
            ...
            <p className="mt-1 text-sm text-ink-secondary">
              Number of land records digitized per district
            </p>
            <div className="mt-4">
